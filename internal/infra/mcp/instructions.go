...
const (
	...
	descListExperts         = "Return the expert directory (members and their expertise tags in `domains`) so the agent can match against its git history to pick the best expert. Every project member is routable — route on the `domains` tags, never on presence/'online' (an expert can answer a question at any time). Each entry also carries `delivery_channel` (where Orako reaches them: discord/slack/teams/telegram/dashboard) and `max_message_chars`, an ADVISORY per-bubble size for that channel (0 = no practical limit, e.g. dashboard). Nothing is truncated: a longer question arrives as several sequential messages, split at paragraph boundaries. Staying within one bubble reads best, so keep questions concise and put the bulk in `context`."
	...
	descGetConversation     = "Fetch a conversation's status and messages by conversation_id. This is how you work the wait ladder after an ask times out — poll it at ~2, then ~5, then ~10 min (space the checks, never tight-loop) — and how you pull the latest when the user says 'check the answer'. Returns the messages so far and whether it's still open. Each message carries `source` (\"human\", \"agent\", or \"system\") and `agent_client`: if the OTHER party's recent messages are `source=agent`, you're talking to another coding agent — proactively OFFER the user to switch to sync mode (the orako_sync prompt) for a fast agent-to-agent back-and-forth, and only enter it once they accept."
